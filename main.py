...
def isShoot(finalres):
    if abs(finalres[15].x * 640 - finalres[16].x * 640) < 100:
        return True
    return False
def isgrenade(finalres):
    if abs(finalres[15].y * 640 ) < 80:
        return True
    return False
def isbend(finalres):
    if abs(finalres[16].y*640)<80:
        return True
    return False
...
while True:
    ...
    if res.pose_landmarks and inFrame(finalres):
        if not (isInit):
            ...
            if isgrenade(finalres):
                cv2.putText(frm, "throwing grenade", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                if not (l_down):
                    l_down = True
                    pyautogui.keyDown("l")
            else:
                cv2.putText(frm, "no grenade", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                if l_down:
                    l_down = False
                    pyautogui.keyUp("l")

            if isbend(finalres):
                cv2.putText(frm, "bend", (50, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                if not (s_down):
                    s_down = True
                    pyautogui.keyDown("s")
            else:
                cv2.putText(frm, "not bending", (50, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                if s_down:
                    s_down = False
                    pyautogui.keyUp("s")
